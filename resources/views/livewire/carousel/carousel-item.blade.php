<div
    class="group relative flex h-full flex-col justify-between overflow-hidden rounded-[2rem] bg-[#FCFBF7] p-4 shadow-sm border border-stone-200/50 transition duration-300 hover:shadow-md">
    <div>
        <!-- Contenedor de Imagen -->
        <div
            class="relative mb-5 aspect-[4/3] w-full overflow-hidden rounded-[1.5rem] bg-[#FCFBF7] flex items-center justify-center p-2">
            {{-- Badge de Descuento --}}
            @if (!empty($item['discount_percentage']))
                <span
                    class="absolute top-3 left-3 z-10 rounded-lg bg-[#8C6D46] px-2.5 py-1 text-[11px] font-semibold text-white shadow-sm">
                    {{ $item['discount_percentage'] }}% OFF
                </span>
            @endif

            {{-- Botón Favorito / Wishlist --}}
            <livewire:components.favorite-button :product-id="$item['id']" wire:key="fav-button-{{ $item['id'] }}" />
    ...
    <!-- Botón Ver Producto -->
    <div class="mt-6 pt-2">
        <button type="button" @click="openProduct(@js($item))"
