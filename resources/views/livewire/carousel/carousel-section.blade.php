<div class="space-y-20 py-8">
    @foreach ($this->carousels as $carousel)
        <section wire:key="carousel-{{ $carousel['id'] }}"
                 ...
                 x-data="{
                     ...
                     move() {
                         ...
                         if (width > gap) {
                             this.$refs.track.style.transform = `translateX(-${this.index * width}px)`;
                         }
                     },

                     scroll(direction, resetTimer = true) {
                         if (resetTimer) this.stopAutoScroll();
                         ...
                         this.move();
                         if (resetTimer) this.startAutoScroll();
                     },

                     startAutoScroll() {
                         this.stopAutoScroll();
                         this.interval = setInterval(() => { this.scroll(1, false); }, 5000);
                     },
                     ...
                     touchEnd(event) {
                         ...
                         const diff = this.touchStartX - end;
            ...
            <div class="relative flex items-center w-full">
                ...
                <!-- Botón Anterior (Se muestra dinámicamente si hay más items que los visibles) -->
                <button type="button"
                            ...
                            <article wire:key="item-{{ $item['id'] }}"
                                ...
                                @if(!empty($item['linkUrl']))
                                    <a href="{{ $item['linkUrl'] }}" class="block h-full">
                                        @include('livewire.carousel.carousel-item', ['item' => $item, 'carousel' => $carousel])
                                    </a>
                                @else
                                    @include('livewire.carousel.carousel-item', ['item' => $item, 'carousel' => $carousel])
                                @endif
                ...
                <!-- Botón Siguiente (Se muestra dinámicamente si hay más items que los visibles) -->
                <button type="button"
                        x-show="total > visible"
                        x-cloak
                        @click="scroll(1)"
