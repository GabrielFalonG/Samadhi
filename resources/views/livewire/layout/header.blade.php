...
<header class="sticky top-0 z-50 w-full bg-[#FAF8F5] text-stone-700 border-b border-stone-200/60 font-sans shadow-sm">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-6">
        <div class="flex items-center justify-between gap-4 py-4 lg:py-6">
            ...
            {{-- Right Actions --}}
            <div class="flex items-center gap-6 text-xs text-stone-600">
                ...
                @auth
                    <div>
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button class="flex items-center gap-2 hover:text-[#A98B68] transition focus:outline-none">
                                    {{-- User Icon --}}
                                    ...
                                    {{-- Caret Arrow Icon --}}
                                    <svg class="h-4 w-4 text-stone-600 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                ...
                @else
                    <div class="flex items-center space-x-6">
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" wire:navigate class="flex items-center gap-2 hover:text-[#A98B68] transition">
                                <svg class="h-5 w-5 text-stone-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                                </svg>
                                <span class="hidden sm:inline">{{ __('Registrarse') }}</span>
                            </a>
                        @endif
                        {{-- Account --}}
                        <a href="{{ route('login') }}" wire:navigate class="flex items-center gap-2 hover:text-[#A98B68] transition">
                            <svg class="h-5 w-5 text-stone-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
