...
@php
    $optionsData = collect($options)
        ->map(function ($option) use ($valueKey, $labelKey, $imageKey) {
            if (is_array($option)) {
                return [
                    'id' => $option[$valueKey],
                    'label' => $option[$labelKey],
                    'image' => $imageKey ? $option[$imageKey] ?? null : null,
                ];
            }
            ...
            return [
                'id' => $option->{$valueKey},
                'label' => $option->{$labelKey},
                'image' => $imageKey ? $option->{$imageKey} : null,
            ];
        })
        ->values();
@endphp
...
<div x-data="{
    ...
    <div @click="open = true"
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            {{-- Pills seleccionados --}}
            <template x-for="id in selected" :key="String(id)">
                <span
                    class="inline-flex items-center gap-1.5 rounded-lg bg-violet-50 px-2.5 py-1 text-sm font-medium text-violet-700">
                    {{-- Imagen SOLO si existe --}}
                    <template x-if="getImage(id)">
                        <img :src="getImage(id)" :alt="getLabel(id)" class="h-6 w-6 rounded-md object-cover">
                    </template>
            ...
            <input type="text" x-model="search" @focus="open = true" @keydown.escape="open = false"
                @keydown.enter.prevent="addCustomOption()" placeholder="{{ $placeholder }}"
            ...
            {{-- Flecha --}}
            <button type="button" @click.stop="open = !open" class="text-slate-400 transition hover:text-slate-600">
                <svg class="h-5 w-5 transition-transform" :class="{ 'rotate-180': open }" fill="none"
    ...
    <div x-show="open" x-transition style="display: none;"
        ...
        <template x-if="allowCreate && search.trim() !== '' && filteredOptions.length === 0">
            <button type="button" @click="addCustomOption()"
        ...
        {{-- Opciones --}}
        <template x-for="option in filteredOptions" :key="String(option.id)">
            <button type="button" @click="toggle(option.id)"
                ...
                <div class="ml-3 flex shrink-0 items-center gap-2">
                    ...
                    <span x-show="isFixed(option.id)" class="text-xs font-normal text-slate-400">
                        Obligatoria
                    </span>

                    {{-- Check --}}
                    <svg x-show="isSelected(option.id)" class="h-5 w-5 text-violet-600" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
